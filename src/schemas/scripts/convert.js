...
const { mkdirpSync, padLeft, uniq, pick, setWith, mapValues } = require('../utils');
...
const categoryEnum = Object.keys(schema.leader.positions.filter(
  ({ start }) => (start === 6),
)[0].values).sort();
const definitions = {};
...
Object.keys(schema['006'].positions).forEach(
  (p) => {
...
definitions['007'] = {
  oneOf: Object.keys(schema)
    .filter((k) => k.substring(0, 3) === '007')
    .map(
      (k) =>
