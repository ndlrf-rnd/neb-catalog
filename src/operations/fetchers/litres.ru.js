 ...
 * Секретный ключ - фиктивный
 */
const LITRES_PARTNER_ID = 'TEST';
const LITRES_SECRET_KEY = '93w4jfhs8imksGo-oa3s85d6Akmkkbnsi9';
const LITRES_FQDN = 'partnersdnld.litres.ru';
...
/**
 * Подписать массив значений по схеме litres используя sha256 hash
 * @param arr
 * @returns {string}
 */
...
/*
  ## Получить файл книги используя "Интерфейс получения файла книги" litres напрямую
...
/*
  ## Получить файл книги с сайта партнера litres (тип 0)
...
/**
 * Эти параетры используются при дистрибьюции через собственный домен партнера.
 */
const LITRES_PARTNER_FQDN = 'litres.rusneb.ru'; /* Пока не заведен */
...
const trial = async (contentType) => {

  /**
   * Пробуем получить ленту всех аудиокниг
   */
  log('Loading content type:', contentType);
  const litresFeedUrl = getLitresNewUrl(contentType, LITRES_EARLIEST_CHECKPOINT);
  ...
  const feedOutputStream = fs.createWriteStream(`./litres-${contentType}_2003-01-01_${getDateStr()}.xml`);
  ...
  const testOutputStream = fs.createWriteStream('/tmp/litres-test-173327.mp3');
  ...
  const bookAudioFileUrl = getFileLitresUrl('37534078-042e-11e1-8be0-47117d41cf4b', '3926885');
  info(`Book file URL: ${bookAudioFileUrl}`);
  const fetchResponse = await fetch(bookAudioFileUrl);
  const bookOutputStream = fs.createWriteStream('/tmp/litres-book.blob');
  await (new Promise((resolve, reject) => {
    bookOutputStream.on('error', reject);
    bookOutputStream.on('end', resolve);
...
trial(process.argv[2] || LITRES_TYPES.AUDIO).catch(error).then(info);
