...
-->
	...
	<xsl:template name="marcRecord">
		...
		<!-- related -->
		<xsl:apply-templates
			select="datafield[500 &lt;= @tag and @tag &lt;= 585]|datafield[700 &lt;= @tag and @tag &lt;= 785]"/>

		<!-- variant -->
		<xsl:apply-templates select="datafield[400 &lt;= @tag and @tag &lt;= 485]"/>

		<!-- notes -->
		<xsl:apply-templates select="datafield[667 &lt;= @tag and @tag &lt;= 688]"/>

		<!-- url -->
		<xsl:apply-templates select="datafield[@tag=856]"/>
		<xsl:apply-templates select="datafield[@tag=010]"/>
		<xsl:apply-templates select="datafield[@tag=024]"/>
		<xsl:apply-templates select="datafield[@tag=372]"/>

		<!-- affiliation-->

		<xsl:for-each select="datafield[@tag=373]">
			<mads:affiliation>
				<mads:position>
		...
		<xsl:for-each select="datafield[@tag=371]">
			<mads:affiliation>
				<mads:address>
					<mads:street>
						<xsl:value-of select="marc:subfield[@code='a']"/>
		...
		<xsl:for-each select="datafield[@tag=374]">
			<mads:extention>
				<mads:profession>
					<mads:professionTerm>
						<xsl:value-of select="marc:subfield[@code='a']"/>
		...
		<xsl:for-each select="datafield[@tag=375]">
			<mads:extension>
				<mads:genderTerm>
					<xsl:value-of select="marc:subfield[@code='a']"/>
				</mads:genderTerm>
		...
		<xsl:for-each select="datafield[@tag=376]">
			<mads:extension>
		...
		<mads:recordInfo>
			...
			<xsl:apply-templates select="datafield[@tag=040]/marc:subfield[@code='a']"/>
			<xsl:apply-templates select="marc:controlfield[@tag=005]"/>
			<xsl:apply-templates select="marc:controlfield[@tag=001]"/>
			<xsl:apply-templates select="datafield[@tag=040]/marc:subfield[@code='b']"/>
			<xsl:apply-templates select="datafield[@tag=040]/marc:subfield[@code='e']"/>
...
-->
	...
	<!-- ========== 372 ========== -->
	<xsl:template match="datafield[@tag=372]">
	...
	<!-- ========== 040 ========== -->
	<xsl:template match="datafield[@tag=040]/marc:subfield[@code='a']">
		<mads:recordContentSource authority="marcorg">
			<xsl:value-of select="."/>
		</mads:recordContentSource>
	</xsl:template>

	<xsl:template match="datafield[@tag=040]/marc:subfield[@code='b']">
	...
	<xsl:template match="datafield[@tag=040]/marc:subfield[@code='e']">
		<mads:descriptionStandard>
			<xsl:value-of select="."/>
		</mads:descriptionStandard>
	</xsl:template>
	...
	<xsl:template match="datafield[@tag=400]">
		<mads:variant>
	...
	<xsl:template match="datafield[@tag=411]">
		<mads:variant>
			<xsl:call-template name="variantTypeAttribute"/>
			<mads:name type="conference">
				<xsl:call-template name="nameACDENQ"/>
	...
	<xsl:template match="datafield[@tag=500]|marc:datafield[@tag=700]">
		<mads:related>
